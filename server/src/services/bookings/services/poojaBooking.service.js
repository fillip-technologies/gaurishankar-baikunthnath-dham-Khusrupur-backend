...
/**
 * Creates a pooja booking and a matching Razorpay order.
 * The price is ALWAYS computed here from the catalogue (pooja.price) — the client
 * only chooses the pooja and quantity, never the amount. Booking depends on
 * payments (it calls createOrderService); payments knows nothing about pooja.
 */
export const createPoojaBookingService = async ({
  poojaId,
  quantity,
  payer,
  ipAddress,
  ...
  const amount = Math.round(pooja.price * quantity * 100);
  if (amount < 100) {
  ...
  const order = await createOrderService({
    ...
    payer,
    purpose: "booking",
    reference: { model: "PoojaBooking", id: booking._id },
    notes: { poojaName: pooja.poojaName, quantity: String(quantity) },
    ipAddress,
    userAgent,
  });
...
/**
 * Lists pooja bookings for the admin dashboard, newest first, with the pooja
 ...
 */
export const getAllPoojaBookingsService = async ({
  status,
  from,
  to,
  page,
